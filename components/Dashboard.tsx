        ...
        <div className="absolute bottom-0 right-0 w-36 h-20 opacity-30 group-hover:opacity-80 transition-all duration-500 ease-out origin-bottom-right group-hover:scale-110 pointer-events-none z-0">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={trendData}>
              ...
              <Area type="monotone" dataKey="value" stroke={activeColor.stroke} strokeWidth={3} fill={`url(#grad-${color})`} isAnimationActive={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
...
const Dashboard: React.FC<DashboardProps> = ({ data, lang, darkMode }) => {
  ...
  // 4. CÁLCULOS RITMO ACTUAL Y PROYECCIÓN (2026)
  const currentTotal = currentYearData?.total || 0;
  
  // Cálculo de días transcurridos para proyección real
  const now = new Date();
  const startOfYear = new Date(now.getFullYear(), 0, 1);
  const diffInMs = now.getTime() - startOfYear.getTime();
  const daysPassed = Math.max(1, Math.floor(diffInMs / (1000 * 60 * 60 * 24)));
  
  const realDailyPace = currentTotal / daysPassed;
  const isLeapYear = (year: number) => (year % 4 === 0 && year % 100 !== 0) || (year % 400 === 0);
  const daysInCurrentYear = isLeapYear(currentSystemYear) ? 366 : 365;
  
  const projectedTotal = realDailyPace * daysInCurrentYear;
  const projectedRemaining = Math.max(0, projectedTotal - currentTotal);

  // Formateador para el Tooltip de la barra fantasma
  const customTooltipFormatter = (value: number, name: string, props: any) => {
    if (name === 'projected') {
      const totalLabel = props?.payload?.projectedTotalLabel || value;
      return [
        `${value.toLocaleString(undefined, { maximumFractionDigits: 1 })} km (Total est: ${totalLabel.toLocaleString(undefined, { maximumFractionDigits: 1 })} km)`, 
        lang === 'es' ? 'Proyección restante' : 'Projected remaining'
      ];
    }
    return [`${value.toLocaleString(undefined, { maximumFractionDigits: 1 })} km`, t.kilometers];
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard title={t.allTimeDistance} value={`${totalAllKm.toLocaleString(undefined, { maximumFractionDigits: 1 })} km`} icon={Trophy} color="blue" subtext="Total acumulado" trendData={trendHistory} />
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {sortedDataDesc.map((d, idx) => (
                <button key={d.year} onClick={() => toggleYear(d.year)} className={`px-3 py-1 rounded-full text-xs font-bold transition-all border ${selectedYears.includes(d.year) ? 'text-white shadow-sm scale-105' : 'bg-transparent text-slate-500 border-slate-200 dark:border-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-500'}`} style={{ backgroundColor: selectedYears.includes(d.year) ? colors[idx % colors.length] : undefined, borderColor: selectedYears.includes(d.year) ? colors[idx % colors.length] : undefined }}>{d.year}</button>
              ))}
            </div>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={monthlyComparisonData}>
                  ...
                  {comparisonDataSrc.map((yearData, idx) => {
                    ...
                    return <Line key={yearData.year} type="monotone" dataKey={yearData.year} name={String(yearData.year)} stroke={colors[originalIdx % colors.length]} strokeWidth={2} dot={{r: 3}} activeDot={{r: 5}} connectNulls />;
                  })}
                </LineChart>
              </ResponsiveContainer>
            </div>
